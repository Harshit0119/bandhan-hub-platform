...
function VendorCardComponent({ vendor, index = 0 }: VendorCardProps) {
  ...
  // ✅ memo derived state (BIG win)
  const favorite = useMemo(() => isFavorite(vendor.id), [isFavorite, vendor.id])

  const handleClick = useCallback(() => {
    console.log("CLICK VENDOR:", vendor)
  ...
  const handleFavoriteClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      favorite ? removeFavorite(vendor.id) : addFavorite(vendor.id)
    },
    [user, router, vendor.id, favorite, addFavorite, removeFavorite]
  )
  ...
  return (
    <motion.div
      initial={index < 8 ? { opacity: 0, y: 15 } : false}
      whileInView={index < 8 ? { opacity: 1, y: 0 } : {}}
      viewport={{ once: true, margin: '50px' }} // ✅ smoother load
    ...
    >
      ...
      >
        ...
        <div className="relative w-full h-64 overflow-hidden bg-muted">
          <Image
            src={imgError ? '/placeholder.webp' : (vendor.profileImage || '/placeholder.webp')}
          ...
          <div className="absolute inset-0 bg-lienear-to-t from-black/60 via-black/10 to-transparent" />
          ...
          <Button
            ...
            className={cn(
              ...
              favorite ? "text-primary" : "text-foreground/60"
            )}
        ...
        <CardContent className="p-4">
          <h3 className="font-serif text-base sm:text-lg font-semibold line-clamp-1 group-hover:text-primary transition-colors">
          ...
          <div className="flex items-center gap-1 text-sm text-muted-foreground mt-2">
            ...
            <span className="line-clamp-1">{vendor.city || 'Location'}</span>
          </div>
        </CardContent>
      </Card>
    </motion.div>
